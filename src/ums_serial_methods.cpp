...
public:
  ...
  virtual std::string format(const log4cpp::LoggingEvent &event) {
    ...
    // 设定自定义格式
    stream << "[" << event.threadName << event.categoryName << event.ndc
           << event.priority << "] " << strTime
           << " [UMS_FICTION_SDK] : " << event.message << "\n";
    return stream.str();
  }
};
...
void UmsSerialMethods::sendEditParamsData() {
  try {
    ...
    root.error("UmsSerialMethods::sendEditParamsData() error: " +
               std::string(e.what()));
  }
}
...
UmsSerialMethods::UmsSerialMethods(const std::string &portName, int baudRate,
                                   bool isDebug, int queueSize,
...
size_t UmsSerialMethods::findElement(const vector<uint8_t> &buffer,
                                     uint8_t element, size_t startPos = 0) {
...
std::vector<uint8_t>
UmsSerialMethods::comFrameReduction(std::vector<uint8_t> &arr) {
...
bool UmsSerialMethods::extractPacket(vector<uint8_t> &buffer,
...
std::string
UmsSerialMethods::magneticDataProcess(const std::vector<uint8_t> &NativeData) {
  std::string MagneticSensorData;
  std::stringstream ss;
  for (uint8_t hexValue : NativeData) {
    ss << std::setw(2) << std::setfill('0') << std::hex
       << static_cast<int>(hexValue);
  }
...
**********************************************************************/
std::vector<uint8_t>
UmsSerialMethods::CompoundVector(std::vector<uint8_t> &byteVector) {
...
**********************************************************************/
std::vector<uint8_t>
UmsSerialMethods::DataDelivery(uint8_t signbit,
...
void UmsSerialMethods::LowerParameterOperationInt(
    const std::string &basicString, uint8_t address, int32_t data,
    const std::shared_ptr<serial::Serial> &Sp) {
  ...
  if (basicString == "write" && Sp != nullptr) {
    ...
    write_in.insert(write_in.end(), datTransformed_date.begin(),
                    datTransformed_date.end());
...
返回  值：无
**********************************************************************/
void UmsSerialMethods::LowerParameterOperation(
    const std::string &red_write, uint8_t address, float data,
    const std::shared_ptr<serial::Serial> &Sp) {
  std::vector<uint8_t> read;
  std::vector<uint8_t> write_in;
  std::vector<uint8_t> datTransformed_date;
...
bool UmsSerialMethods::DataCheck(std::vector<uint8_t> &data) {
  ...
  if (result_h == data[data.size() - 3] && result_l == data[data.size() - 2]) {
    ...
    root.debug("high: %02x low: %02x", data[data.size() - 3],
               data[data.size() - 2]);
...
返回  值：double result
**********************************************************************/
double UmsSerialMethods::DirectionalInterception(
    int startIndex, int count, const std::vector<uint8_t> &byteData) {
  size_t startIndexs = startIndex;
  size_t counts = count;
  std::vector<uint8_t> subVector(byteData.begin() + startIndexs,
                                 byteData.begin() + startIndexs + counts);
...
PowerInfo
UmsSerialMethods::PowerDataProcess(const std::vector<uint8_t> &NativeData) {
  // 总线电流
  ...
  try {
    ...
    if (ImuData[4] == 0x04 && runtimeVersion == AgreementVersion::V2) {
      for (int i = 0; i < 13; i = i + 1) {
        ...
        subVector.insert(subVector.begin(), ImuData.begin() + 5 + (i * 8),
                         ImuData.begin() + 13 + (i * 8));
      ...
      //            ImuStructural.axaxis, ImuStructural.ayaxis,
      //            ImuStructural.azaxis, ImuStructural.gxaxis,
      //            ImuStructural.gyaxis, ImuStructural.gzaxis,
      //            ImuStructural.pitch, ImuStructural.roll);
    } else if (runtimeVersion == AgreementVersion::V1) {
      std::vector<uint8_t> subVector;
      if (ImuData[4] == 0x00) {
        subVector.insert(subVector.begin(), ImuData.begin() + 5,
                         ImuData.begin() + 13);
        ImuStructural.axaxis = BinaryToDouble(subVector);
        subVector.clear();

        subVector.insert(subVector.begin(), ImuData.begin() + 13,
                         ImuData.begin() + 21);
        ImuStructural.ayaxis = BinaryToDouble(subVector);
        subVector.clear();

        subVector.insert(subVector.begin(), ImuData.begin() + 21,
                         ImuData.begin() + 29);
        ...
        subVector.insert(subVector.begin(), ImuData.begin() + 29,
                         ImuData.begin() + 37);
...
void UmsSerialMethods::createSerial(const std::string &portName, int baudRate) {
  while (!stopFlag) {
    try {
      sp = std::make_shared<serial::Serial>(
      ...
      sp = nullptr;
      root.error("Exception thrown: " + std::string(e.what()) +
                 "  port:" + portName);
      sleep(3);
    }
  }
...
OdomInfo
UmsSerialMethods::OdomDataProcess(const std::vector<uint8_t> &ImuData) {
...
MotorFeedbackInfo UmsSerialMethods::MotorFeedbackDataProcess(
    const std::vector<uint8_t> &motorData) {
  MotorFeedbackInfo result{};

  try {
    // 协议格式: 0x53 | 长度 | 电机序号 | 目标速度(8B) | 实际速度(8B) |
    // 输出功率(8B) motorData[4] = 电机序号 (0x00-0x03) motorData[5-12] =
    // 目标速度 (8 bytes double) motorData[13-20] = 实际速度 (8 bytes double)
    // motorData[21-28] = 输出功率 (8 bytes double)

    if (motorData.size() >= 29) {
      // 提取目标速度 (8 bytes starting at index 5)
      result.targetSpeed = DirectionalInterception(5, 8, motorData);

      // 提取实际速度 (8 bytes starting at index 13)
      result.actualSpeed = DirectionalInterception(13, 8, motorData);

      // 提取输出功率 (8 bytes starting at index 21)
      result.outputPower = DirectionalInterception(21, 8, motorData);
    }
  } catch (const std::exception &e) {
    root.error("MotorFeedbackDataProcess exception: " + std::string(e.what()));
  }
...
bool UmsSerialMethods::ParamDataWrite() {
  try {
    if (sp->isOpen()) {
      ...
      if (inputParam.LB != 0)
        LowerParameterOperation(LC_read_write, 32, inputParam.LB, sp);
...
ICDRemote UmsSerialMethods::convertBackDataToControl(int channel1Value,
                                                     int channel2Value,
                                                     int channel3Value) {
  ...
  // 自转角速度转换
  float yawRate = ((float)channel1Value / maxOutputValue) *
                  180; // 假设最大自转角速度为±180度/秒

  // Vx线速度转换
  float vxSpeed =
      ((float)channel2Value / maxOutputValue) * 1; // 假设Vx的最大速度为±1m/s

  // Vy线速度转换
  float vySpeed =
      ((float)channel3Value / maxOutputValue) * 1; // 假设Vy的最大速度为±1m/s
...
RCSBUSRemote
UmsSerialMethods::convertRCBusRemote(std::vector<uint8_t> &byteVector) {
  RCSBUSRemote rcsbusRemote{};
  rcsbusRemote.len = byteVector[3];
  for (int index = 0; index < 8; index = index + 2) {
    uint16_t result = (static_cast<uint16_t>(byteVector[index + 5]) << 8) |
                      byteVector[index + 4];
    rcsbusRemote.axes[index / 2] = result;
  }
  return rcsbusRemote;
}
void UmsSerialMethods::loopUmsFictionData(
    const std::shared_ptr<FictionData> &FictionData) {
  rdThread = std::thread(&UmsSerialMethods::tdLoopUmsFictionData, this, sp,
                         FictionData);
...
void UmsSerialMethods::monitorTimeout() {
  while (!stopFlag) {
    if (std::chrono::steady_clock::now() - lastReceiveTime >
    ...
    if (sp != nullptr) {
      while (!stopFlag) {
        ...
        if (!circularQueue->enqueue(data)) {
          // enqueue a false return value, which means that the queue has been
          // stopped.
          if (stopFlag)
            break;
        }
...
void UmsSerialMethods::tdLoopUmsFictionData(
    const std::shared_ptr<serial::Serial> &Sp,
    const std::shared_ptr<FictionData> &FictionData) {
  lastReceiveTime = std::chrono::steady_clock::now();
  timeoutOccurred = false;
  ...
  try {
    if (FictionData != nullptr) {
      ...
      while (!stopFlag) {
        ...
        if (circularQueue->dequeue(data)) {
          ...
          if (!packet.empty()) {
            if (packet[1] == 0x3a && packet[packet.size() - 1] == 0x0a &&
                packet[0] == 0x0d && checkSignValue(packet[2]) &&
              ...
              if (DataCheck(packet)) {
                ...
                {
                  FictionData->icdData =
                      convertBackDataToControl(packet[4], packet[5], packet[6]);
                  break;
                }
                case 0x4b: // 里程计
                {
                ...
                {
                  // 获取电机序号 (0x00-0x03)
                  uint8_t motorNum = packet[4];
                  if (motorNum < 4) {
                    FictionData->motorFeedback[motorNum] =
                        MotorFeedbackDataProcess(packet);
                  }
                  break;
                }
                case 0x54: // 数字式温度传感器
                  FictionData->temperature =
                      static_cast<float>((packet[5] << 8) | packet[4]);
                  break;
                case 0x55: // 超声数据
                  FictionData->ultrasonic =
                      static_cast<float>((packet[5] << 8) | packet[4]);
                  break;
                default:
                  // 处理默认情况
          ...
          // If dequeue returns false, it indicates that the queue has been
          // stopped and is empty, and the loop will be ready to exit.
          if (stopFlag) {
            break;
          }
        }
      ...
      //                reThread.detach();
      //            }
      //            reThread = std::thread(&UmsSerialMethods::reStartSerial,
      //            this, sp->getPort(), sp->getBaudrate());
      //            if(timeoutOccurred){
      //                root.warn("serial port data:  %ld , timeout",
      //                sp->available());
      //            }
    ...
    if (reThread.joinable())
      reThread.detach();
    reThread = std::thread(&UmsSerialMethods::reStartSerial, this,
                           sp->getPort(), sp->getBaudrate());
  }
}

std::vector<uint8_t> UmsSerialMethods::generatePwmPacket(uint8_t channel,
                                                         uint16_t pulseWidth) {
  // --- 1. 输入参数校验 ---
  if (channel > 7) {
    std::cerr << "Error: Channel number " << static_cast<int>(channel)
              << " is out of the valid range [0, 7]." << std::endl;
    return {}; // 返回空vector表示失败
  }
  if (pulseWidth > 50000) {
    std::cerr << "Error: Pulse width " << pulseWidth
              << " is out of the valid range [0, 50000]." << std::endl;
    return {}; // 返回空vector表示失败
  }
  // --- 2. 创建协议数据包 ---
...
int32_t UmsSerialMethods::generateUplhzPacket(uint16_t frequency,
                                              bool enableUsbCom,
                                              bool enableCom1,
                                              bool enableCom2) {
  if (frequency > 1000) {
    std::cerr << "Error: Frequency " << frequency
              << " is out of the valid range [0, 1000]." << std::endl;
    return -1; // 返回-1表示错误
  }
...
void UmsSerialMethods::sendTwistData(
    const std::shared_ptr<TwistCustom> &twistData) {
  ...
  if (sp != nullptr && twistData != nullptr && !stopFlag) {
    ...
    SendHexData.insert(SendHexData.end(), LeftWheelSpeed.begin(),
                       LeftWheelSpeed.end());
    SendHexData.insert(SendHexData.end(), RightWheelSpeed.begin(),
                       RightWheelSpeed.end());
    SendHexData.insert(SendHexData.end(), AngularVelocity.begin(),
                       AngularVelocity.end());
...
void UmsSerialMethods::reStartSerial(const std::string &portName,
                                     int baudRate) {
  stopFlag = true;
  if (spThread.joinable()) {
    spThread.join(); // 确保之前的线程已经完成
  ...
  stopFlag = false;
  spThread =
      std::thread(&UmsSerialMethods::createSerial, this, portName, baudRate);
  if (spThread.joinable())
    spThread.join();
