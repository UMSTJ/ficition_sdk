...
struct ParamsData {
  ...
  bool operator==(const ParamsData &other) const {
    ...
    return KP == other.KP && KI == other.KI && KD == other.KD &&
           ...
           MPC == other.MPC && KMTT == other.KMTT &&
           IMU_ZOFS == other.IMU_ZOFS && IMU_YOFS == other.IMU_YOFS &&
           IMU_XOFS == other.IMU_XOFS && MTDIR == other.MTDIR &&
           MSNUM == other.MSNUM && ROSID == other.ROSID &&
           UPLHZ == other.UPLHZ && FUNC1 == other.FUNC1 &&
           FUNC2 == other.FUNC2 && FUNC3 == other.FUNC3 &&
...
enum class AgreementVersion : int { V1, V2 };

struct MotorFeedbackInfo {
  double targetSpeed; // 目标速度 m/s
  double actualSpeed; // 实际速度 m/s
  double outputPower; // 输出功率 0~1 (0~100%)
};
...
struct FictionData {
  ...
  MotorFeedbackInfo motorFeedback[4]; // 四个电机的反馈数据
};
