...
public:
   ...
   * 包含生成的数据包。如果输入参数无效，则返回一个空vector。
   */
  ...
  UmsSerialMethods();
  UmsSerialMethods(const std::string &portName, int baudRate, bool isDebug,
                   int queueSize, AgreementVersion agreementVersion);

  // 添加状态查询方法
  bool isRunning() const { return !stopFlag; }

  // 修改析构函数
  ~UmsSerialMethods() { stop(); }
...
private:
  ...
  bool ParamDataWrite();
  // ICD
  ICDRemote convertBackDataToControl(int channel1Value, int channel2Value,
                                     int channel3Value);
  // RCBUS
  RCSBUSRemote convertRCBusRemote(std::vector<uint8_t> &byteVector);

  void tdLoopUmsFictionData(const std::shared_ptr<serial::Serial> &Sp,
   ...
   */
  int32_t generateUplhzPacket(uint16_t frequency, bool enableUsbCom,
                              bool enableCom1, bool enableCom2);
  ...
  **********************************************************************/
  ...
  返回  值：无
  **********************************************************************/
  std::vector<uint8_t> DataDelivery(uint8_t signbit,
                                    const std::vector<uint8_t> &Vector);

  void LowerParameterOperationInt(const std::string &basicString,
                                  uint8_t address, int32_t data,
  ...
  返回  值：无
  **********************************************************************/
  void LowerParameterOperation(const std::string &red_write, uint8_t address,
                               float data,
                               const std::shared_ptr<serial::Serial> &Sp);
  ...
  **********************************************************************/
  ...
  返回  值：double result
  **********************************************************************/
  double DirectionalInterception(int startIndex, int count,
  ...
  **********************************************************************/
  ...
  MotorFeedbackInfo
  MotorFeedbackDataProcess(const std::vector<uint8_t> &motorData);
  ...
  vector<uint8_t> signedValue = {0x41, 0x42, 0x45, 0x51, 0x52, 0x46,
                                 0x49, 0x4b, 0x53, 0x54, 0x55};
  ...
  size_t findElement(const vector<uint8_t> &buffer, uint8_t element,
                     size_t startPos);
